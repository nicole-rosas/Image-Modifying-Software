...
/**
 * Mock for EnhanceControllerBasic to be used for testing.
 */
public class EnhanceControllerBasicMock implements ImageEnhanceControllerInterface {
  ...
  /**
   * Empty constructor for EnhanceControllerBasicMock.
   */
  public EnhanceControllerBasicMock() {
  ...
  /**
   * Constructor for EnhanceControllerBasicMock that takes in a MultiLayeredEnhanceInterface,
   * a readable, and an appendable.
   * @param model an MultiLayeredEnhanceInterface
   * @param in a Readable
   * @param out an Appendable
   */
  public EnhanceControllerBasicMock(MultiLayeredEnhanceInterface model,
      Readable in, Appendable out, String batch) {
    ...
    while (scan.hasNext()) {
      ...
      if (cmd == null) {
        ...
        try {
          ...
          this.writeMessage("The command that you intend to run is invalid. Try again.\n");
        }
      }
    }
  }
  ...
  private ChangeType getChangeType(String type) {
    switch (type) {
      case "current": return ChangeType.CURRENT;
      case "name": return ChangeType.NAME;
      default: throw new IllegalArgumentException("Not a valid change type.");
    }
  }
  ...
  private ExportType getExportType(String type) {
    switch (type) {
      case "ppm": return ExportType.PPM;
      case "png": return ExportType.PNG;
      case "jpeg": return ExportType.JPEG;
      default: throw new IllegalArgumentException("Not a valid export type.");
    }
  }
  ...
  private ImportType getImportType(String type) {
    switch (type) {
      case "ppm": return ImportType.PPM;
      case "png": return ImportType.PNG;
      case "jpeg": return ImportType.JPEG;
      default: throw new IllegalArgumentException("Not a valid import type.");
    }
  }
  ...
  private KernelType getKernelType(String type) {
    switch (type) {
      case "filter": return KernelType.BLUR;
      case "sharpen": return KernelType.SHARPEN;
      default: throw new IllegalArgumentException("Not a valid kernel/filter type.");
    }
  }
  ...
  private TransformType getTransformType(String type) {
    switch (type) {
      case "sepia": return TransformType.SEPIA;
      case "greyscale": return TransformType.GREYSCALE;
      default: throw new IllegalArgumentException("Not a valid kernel/filter type.");
    }
  }
  ...
  /**
   * Generates Map of Scanned and EnhanceCommandInterface which will determine when methods
   * appear.
